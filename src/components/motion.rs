...
pub fn VisualMotion(motion: stela::VisualMotion) -> impl IntoView {
    ...
    let (text, set_text) = create_signal(motion.title.clone());
            ...
            <a
                ...
                on:click:undelegated={
                    ...
                    move |ev| {
                        ev.prevent_default();
                        let data = data.clone();
                        let old_toggle = is_toggled.get_untracked();
                        ...
                        spawn_local(async move {
                            ...
                            match res {
                                Ok(res) => {
                                    ...
                                    if let Some(new_text) = res.new_text.clone() {
                                        set_text.try_set(Some(new_text));
                                    }
                                }
                                Err(_) => {
            ...
            view! {
                {
                    ...
                    move || {
                        if can_share.get() {
                            view! {
                                <button
                                    class=["stela--visual-motion ", variant_class, " ", color_class]
                                        .concat()
                                    on:click={
                                        let share_data = share_data.clone();
                                        move |_| {
