...
pub fn Page(page: Arc<stela::Page>) -> impl IntoView {
    ...
    let meta_tags = page.social.clone().map(|social| {
        ...
        view! {
            ...
            {social.image.as_ref().map(|image| view! {
            ...
            {social.url.as_ref().map(|url| view! {
                <Meta property="og:url" content=url.clone()/>
            })}
            {social.twitter_card.as_ref().map(|card| view!{
                <Meta name="twitter:card" content=card.clone()/>
            })}
        }
    });
